<script>
	...
	function deleteTreatment(name) {
		s.update(state => ({
			...state,
			treatments: state.treatments.filter(treatment => treatment !== name)
		}));
	}
</script>
	...
	<div class="card-body">
		...
		<table class="table table-striped table-hover">
			...
			<tbody>
				...
				<tr>
					<th scope="row">{treatment} <button class="btn btn-danger btn-sm" onclick={() => deleteTreatment(treatment)}>Delete</button></th>
				</tr>
				{/each}
			</tbody>
		</table>
